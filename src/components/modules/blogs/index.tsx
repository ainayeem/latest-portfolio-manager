...
export default function ManageBlog({ blogs }: { blogs: TBlog[] }) {
  console.log("🚀 ~ ManageBlog ~ blogs:", blogs);
  ...
  const columns: ColumnDef<TBlog>[] = [
    ...
    },
    {
      ...
      header: "Author",
      cell: ({ row }) => <div className="font-medium capitalize">{row.getValue("authorName")}</div>,
    },
    // {
        ...
        return (
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end">
              ...
              {/* <DropdownMenuItem>
                <Link href={`/blogs/update-blog/${blog?._id}`} className="flex gap-2">
                  <FaEdit className="mr-2 text-amber-500" /> Edit
                </Link>
              </DropdownMenuItem> */}

              <DropdownMenuItem onClick={() => handleDeleteBlog(blog?._id)} className="cursor-pointer">
        ...
        <Table>
          ...
          <TableBody>
            {table.getRowModel().rows.length ? (
              ...
              <TableRow>
                <TableCell colSpan={columns.length} className="h-24 text-center text-[#989BA4]">
                  No blogs data found.
                </TableCell>
              </TableRow>
            )}
